...
void chess_board_line::create(const vulkan_application* _app, vk::SampleCountFlagBits _multisample_count, vk::Format _color_formats, vk::Format _depth_format)
{
	...
	if (!MODEL_LOADER.load_model(std::u8string(MODELS_PATH) + u8"chess_board_line.glb", vertices, indices))
	{
	...
	for (uint32_t i = 0; i < vulkan_common::MAX_FRAMES_IN_FLIGHT; i++)
	{
		...
		buffer.create(_app->get_physical_device(), _app->get_device(), sizeof(chess_board_line::UBO), vk::BufferUsageFlagBits::eUniformBuffer | vk::BufferUsageFlagBits::eStorageBuffer, vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent);
...
void chess_board_line::resize(const vulkan_application* _app, uint32_t _width, uint32_t _height)
{
	...
	auto pool_info = ubos | std::views::transform([](const auto& _buffer) -> DescriptorBufferOrImageInfo
		{
			return vk::DescriptorBufferInfo(_buffer.get_buffer(), 0, sizeof(chess_board_line::UBO));
		})
		| std::ranges::to<std::vector>();

	descriptor.add_descriptor_info(vk::DescriptorType::eUniformBuffer, std::move(pool_info));
...
{
	return std::vector<vk::AccelerationStructureInstanceKHR>{ vk::AccelerationStructureInstanceKHR(vulkan_common::glm_matrix_to_vulkan(glm::translate(glm::mat4(1.f), glm::vec3(0.f, 0.f, 0.15f))), 1, 0xFF, 0, vk::GeometryInstanceFlagBitsKHR::eTriangleCullDisable, blas.get_address()) };
}
