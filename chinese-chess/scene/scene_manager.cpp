    ...
    , recycle_bin(_app.get_recycle_bin())
{
    ...
    computr_queue.create(*app.get_device(), app.get_physical_device().get_queue_index(vk::QueueFlagBits::eCompute));
    transfer_queue.create(*app.get_device(), app.get_physical_device().get_queue_index(vk::QueueFlagBits::eTransfer));


    material_manager = std::make_unique<scene_material_manager>(app, recycle_bin, graphic_queue, transfer_queue);
    model_manager    = std::make_unique<scene_model_manager>(app, recycle_bin, *material_manager, graphic_queue,
                                                             computr_queue, transfer_queue);
    light_manager    = std::make_unique<scene_light_manager>(app, recycle_bin, graphic_queue, transfer_queue);
...
void scene_manager::update()
{
    ...
    update_rasterization(wait_infos);
    update_ray_tracing(wait_infos);
}
...
void scene_manager::create_rasterization()
{
    ...
    auto spirv_code = SHADER_COMPILER.compile_shader_to_spv(std::u8string(SHADERS_PATH) + u8"rasterization.slang",
    ...
    vk::raii::ShaderModule shaderModule(*app.get_device(),
    ...
    raster_pipeline.create(*app.get_device(), bindings, std::span(&push_constant, 1), std::span(&binding, 1), attribute,
                           shader_stages, vk::PrimitiveTopology::eTriangleList, vk::PolygonMode::eFill,
                           vk::CullModeFlagBits::eBack, vk::FrontFace::eCounterClockwise, vulkan_common::MSAA_SAMPLE_COUNT,
                           vk::True, std::span(&color_format, 1), vulkan_common::DEPTH_FORMAT);
}
...
void scene_manager::resize_rasterization()
{
    ...
    vk::ImageCreateInfo color_image_info({}, vk::ImageType::e2D, color_format, vk::Extent3D(width, height, 1), 1, 1,
    ...
    vk::ImageViewCreateInfo color_view_info({}, {}, vk::ImageViewType::e2D, color_format, {},
                                            vk::ImageSubresourceRange(vk::ImageAspectFlagBits::eColor, {}, 1, 0, 1), nullptr);
    raster_color_image.create(app.get_allocator(), *app.get_device(), color_image_info, color_view_info,
                              vk::MemoryPropertyFlagBits::eDeviceLocal, vk::ClearColorValue(0.f, 0.f, 0.f, 1.f));
    ...
    raster_depth_image.create(app.get_allocator(), *app.get_device(), depth_image_info, depth_view_info,
                              vk::MemoryPropertyFlagBits::eDeviceLocal, vk::ClearDepthStencilValue(1.f, 0));
}

void scene_manager::update_rasterization(std::vector<vk::SemaphoreSubmitInfo>& _pending_waits)
{
    if (!model_manager->get_models().empty())
    {
        // update draw commands
        auto draw_commands = model_manager->get_models() | std::views::transform([](const auto& p) {
                                 auto sp = p.lock();
                                 return vk::DrawIndexedIndirectCommand(
                                     static_cast<uint32_t>(sp->model_info->indices.size()), 1,
        ...
        vulkan_commandbuffer commandbuffer =
            std::move(vulkan_commandbuffer::create(*app.get_device(),
                                                   vk::CommandBufferAllocateInfo(transfer_queue.get_command_pool(),
                                                                                 vk::CommandBufferLevel::ePrimary, 1),
                                                   &transfer_queue, app.get_semaphore_ptr())
                          .front());
        ...
        raster_draw_commands.create(app.get_allocator(), *app.get_device(),
                                    vk::BufferCreateInfo({}, draw_commands_size,
        ...
        draw_commands_staging_buffer.create(app.get_allocator(), *app.get_device(),
                                            vk::BufferCreateInfo({}, draw_commands_size, vk::BufferUsageFlagBits::eTransferSrc,
                                                                 vk::SharingMode::eExclusive, queue_array),
                                            vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent);

        memcpy(draw_commands_staging_buffer.get_buffer_address().hostAddress, draw_commands.data(), draw_commands_size);
        vulkan_buffer::copy_buffer_to_buffer(*commandbuffer, draw_commands_staging_buffer.get_buffer(),
                                             raster_draw_commands.get_buffer(), vk::BufferCopy2(0, 0, draw_commands_size));

        commandbuffer.end_record();
        commandbuffer.submit(false);

        _pending_waits.push_back(commandbuffer.get_submit_info());

        recycle_bin.retire(std::move(draw_commands_staging_buffer));
    ...
    // descriptor set
    std::vector<vk::DescriptorSetLayout> layouts(vulkan_common::MAX_FRAMES_IN_FLIGHT, raster_pipeline.get_descriptor_set_layout());
    ...
    std::ranges::for_each(raster_descriptor_sets | std::views::enumerate, [&](const auto& _pair) {
        ...
        auto       material_sets = material_manager->get_descriptor_info(sampler) | std::views::enumerate
...
void scene_manager::render_rasterization(const vk::raii::CommandBuffer& _commandbuffer) noexcept
{
    ...
    auto raster_color_barrier =
        vk::ImageMemoryBarrier2(raster_color_image.get_stage(), raster_color_image.get_access(),
                                vk::PipelineStageFlagBits2::eColorAttachmentOutput, vk::AccessFlagBits2::eColorAttachmentWrite,
                                raster_color_image.get_layout(), vk::ImageLayout::eColorAttachmentOptimal,
                                vk::QueueFamilyIgnored, vk::QueueFamilyIgnored, raster_color_image.get_image(),
                                vk::ImageSubresourceRange(vk::ImageAspectFlagBits::eColor, 0, 1, 0, 1));
    raster_color_image.set_info(raster_color_barrier);

    auto raster_depth_barrier =
        vk::ImageMemoryBarrier2(raster_depth_image.get_stage(), raster_depth_image.get_access(),
                                vk::PipelineStageFlagBits2::eAllGraphics, vk::AccessFlagBits2::eDepthStencilAttachmentWrite,
                                raster_depth_image.get_layout(), vk::ImageLayout::eDepthAttachmentOptimal,
                                vk::QueueFamilyIgnored, vk::QueueFamilyIgnored, raster_depth_image.get_image(),
                                vk::ImageSubresourceRange(vk::ImageAspectFlagBits::eDepth, 0, 1, 0, 1));
    raster_depth_image.set_info(raster_depth_barrier);

    const std::array begin_barriers = {render_output_barrier, raster_color_barrier, raster_depth_barrier};
    _commandbuffer.pipelineBarrier2(vk::DependencyInfo({}, {}, {}, begin_barriers, nullptr));


    vk::RenderingAttachmentInfo colorAttachmentInfo(raster_color_image.get_imageview(), vk::ImageLayout::eColorAttachmentOptimal,
                                                    ...
                                                    vk::AttachmentStoreOp::eStore, raster_color_image.get_clear_value());
    vk::RenderingAttachmentInfo depthAttachmentInfo(raster_depth_image.get_imageview(), vk::ImageLayout::eDepthStencilAttachmentOptimal,
                                                    vk::ResolveModeFlagBits::eNone, {}, vk::ImageLayout::eUndefined,
                                                    vk::AttachmentLoadOp::eClear, vk::AttachmentStoreOp::eDontCare,
                                                    raster_depth_image.get_clear_value());

    vk::RenderingInfo renderingInfo({}, vk::Rect2D({0, 0}, {static_cast<uint32_t>(width), static_cast<uint32_t>(height)}),
    ...
    _commandbuffer.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, raster_pipeline.get_pipeline_layout(), 0,
                                      *(raster_descriptor_sets.at(current_frame)), nullptr);
    ...
    _commandbuffer.pushConstants2(vk::PushConstantsInfo(raster_pipeline.get_pipeline_layout(), vk::ShaderStageFlagBits::eVertex,
                                                        0, sizeof(scene_manager::push_constant), &pc));
    ...
    if (!model_manager->get_models().empty()) [[likely]]
    {
        ...
        _commandbuffer.drawIndexedIndirect(raster_draw_commands.get_buffer(), 0,
...
void scene_manager::create_ray_tracing()
{
    ...
    auto spirv_code = SHADER_COMPILER.compile_shader_to_spv(std::u8string(SHADERS_PATH) + u8"ray_tracing.slang",
                                                            {RAY_GEN_ENTYR_NAME, RAY_MISS_ENTYR_NAME, RAY_SHADOW_MISS_ENTYR_NAME,
                                                             RAY_CLOSEST_HIT_ENTRY_NAME, RAY_SHADOW_ANY_HIT_ENTYR_NAME});
    if (spirv_code.empty())
    {
        throw std::runtime_error("compile .spv failed!");
    }
    ...
    std::array<vk::PipelineShaderStageCreateInfo, static_cast<size_t>(stage_indices::shader_group_max_count)> shader_stages = {
        ...
        vk::PipelineShaderStageCreateInfo({}, vk::ShaderStageFlagBits::eMissKHR, shaderModule,
                                          RAY_SHADOW_MISS_ENTYR_NAME.data()),
        ...
        vk::PipelineShaderStageCreateInfo({}, vk::ShaderStageFlagBits::eAnyHitKHR, shaderModule,
                                          RAY_SHADOW_ANY_HIT_ENTYR_NAME.data()),
    };
    ...
    auto props =
        (*app.get_physical_device()).getProperties2<vk::PhysicalDeviceProperties2, vk::PhysicalDeviceRayTracingPipelinePropertiesKHR, vk::PhysicalDeviceAccelerationStructurePropertiesKHR>();
    const auto& properties = props.get<vk::PhysicalDeviceRayTracingPipelinePropertiesKHR>();
    rt_pipeline.create(*app.get_device(), bindings, std::span(&push_constant, 1), shader_stages, shader_groups,
                       std::min(9u, properties.maxRayRecursionDepth));
    ...
    auto staging_buffer =
        rt_sbt.create(*app.get_physical_device(), *app.get_device(), app.get_allocator(), *commandbuffer,
                      rt_pipeline.get_pipeline(), static_cast<uint32_t>(shader_stages.size()), graphic_queue.get_index());
...
void scene_manager::update_ray_tracing(std::vector<vk::SemaphoreSubmitInfo>& _pending_waits)
{
    // reset path tracing accumulation
    rt_frame_index = 0;

    recycle_bin.retire(std::move(rt_tlas));
    rt_tlas.resize(vulkan_common::MAX_FRAMES_IN_FLIGHT);

    if (!model_manager->get_models().empty())
    {
        // generate tlas (graphics queue required: AS build needs VK_QUEUE_COMPUTE_BIT)
        vulkan_commandbuffer commandbuffer =
            std::move(vulkan_commandbuffer::create(*app.get_device(),
                                                   vk::CommandBufferAllocateInfo(graphic_queue.get_command_pool(),
                                                                                 vk::CommandBufferLevel::ePrimary, 1),
        ...
        std::ranges::for_each(rt_tlas, [&](auto& _tlas) {
            ...
            const std::array queue_array = {graphic_queue.get_index()};
            instance_staging_buffer.create(app.get_allocator(), *app.get_device(),
                                           vk::BufferCreateInfo({}, instance_buffer_size,
            ...
            staging_buffer.create(app.get_allocator(), *app.get_device(),
                                  vk::BufferCreateInfo({}, instance_buffer_size, vk::BufferUsageFlagBits::eTransferSrc,
                                                       vk::SharingMode::eExclusive, queue_array),
                                  vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent);

            memcpy(staging_buffer.get_buffer_address().hostAddress, rt_instances.data(), instance_buffer_size);
            vulkan_buffer::copy_buffer_to_buffer(*commandbuffer, staging_buffer.get_buffer(), instance_staging_buffer.get_buffer(),
                                                 vk::BufferCopy2(0, 0, instance_buffer_size));

            auto scratch_buffer = _tlas.create_top_level_acceleration_structure(
                *app.get_physical_device(), *app.get_device(), app.get_allocator(), *commandbuffer,
                static_cast<uint32_t>(rt_instances.size()), instance_staging_buffer.get_buffer_address().deviceAddress,
    ...
    // descriptor set
    std::vector<vk::DescriptorSetLayout> layouts(vulkan_common::MAX_FRAMES_IN_FLIGHT, rt_pipeline.get_descriptor_set_layout());
    ...
    std::ranges::for_each(rt_descriptor_sets | std::views::enumerate, [&](const auto& _pair) {
        ...
        vk::DescriptorBufferInfo as_buffer_info(rt_tlas.at(index).get_buffer(), 0,
                                                sizeof(vk::AccelerationStructureInstanceKHR)
                                                    * model_manager->get_models().size());
...
void scene_manager::render_ray_tracing(const vk::raii::CommandBuffer& _commandbuffer) noexcept
{
    ...
    _commandbuffer.bindDescriptorSets(vk::PipelineBindPoint::eRayTracingKHR, rt_pipeline.get_pipeline_layout(), 0,
                                      *(rt_descriptor_sets.at(current_frame)), nullptr);
    ...
    _commandbuffer.pushConstants2(vk::PushConstantsInfo(rt_pipeline.get_pipeline_layout(), vk::ShaderStageFlagBits::eAll,
                                                        0, sizeof(scene_manager::push_constant), &pc));

    _commandbuffer.traceRaysKHR(rt_sbt.get_raygen_region(), rt_sbt.get_miss_region(), rt_sbt.get_hit_region(),
                                rt_sbt.get_callable_region(), width, height, 1);

    auto image_end_barrier =
        vk::ImageMemoryBarrier2(render_output.get_stage(), render_output.get_access(),
