...
static void key_callback(GLFWwindow* window, int key, int /*scancode*/, int action, int /*mods*/)
{
    ...
    {
        static_cast<window_info*>(glfwGetWindowUserPointer(window))->ui->handle(key);
...
int main()
{
    ...
    std::unique_ptr<scene_manager> scene =
        std::make_unique<scene_manager>(*app, static_cast<uint32_t>(width), static_cast<uint32_t>(height));

    // create ui
    std::unique_ptr<ui_manager> UI =
        std::make_unique<ui_manager>(window, *app, *scene, static_cast<uint32_t>(width), static_cast<uint32_t>(height));
    info.ui = UI.get();
