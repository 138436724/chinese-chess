...
bool model_loader::load_model(const std::filesystem::path& _file_path, std::vector<model_vertex>& _vertices, std::vector<uint32_t>& _indices) noexcept
{
	...
	for (auto& _mesh : gltf.meshes)
	{
		for (auto&& _primitives : _mesh.primitives)
		{
			auto initial_vtx = static_cast<uint32_t>(_vertices.size());

			fastgltf::Accessor& index_accessor = gltf.accessors.at(_primitives.indicesAccessor.value());
			...
			auto uvs = _primitives.findAttribute("TEXCOORD_0");
