...
void vulkan_device::create(const vk::raii::PhysicalDevice& _physical_device, const std::span<uint32_t const> _queues, const std::span<const char* const> _extensions, const vk::PhysicalDeviceFeatures2& _features)
{
	...
	auto device_queue_create_info = all_queues
		| std::views::transform(
			[&queue_priority](const auto& index)
			{
