...
void vulkan_application::create(vk::SurfaceKHR _surface, bool _enable_graphics, bool _enable_compute, uint32_t _width, uint32_t _height)
{
	...
	const vulkan_queue& the_queue = get_queue(vk::QueueFlagBits::eGraphics);
	primary_commandbuffers = vulkan_commandbuffer::create(vk::CommandBufferAllocateInfo(the_queue.get_command_pool(), vk::CommandBufferLevel::ePrimary, vulkan_common::MAX_FRAMES_IN_FLIGHT), &device, &(the_queue.get_queue()));
	secondary_commandbuffers = vulkan_commandbuffer::create(vk::CommandBufferAllocateInfo(the_queue.get_command_pool(), vk::CommandBufferLevel::eSecondary, vulkan_common::MAX_FRAMES_IN_FLIGHT * 2), &device, &(the_queue.get_queue()));
}
...
void vulkan_application::begin_frame()
{
	...
	try
	{
		auto res = swapchain.acquire_next_image();
		acquire_result = res.first;
		acquire_semaphore = res.second;
	}
	...
	primary_commandbuffers.at(current_frame).begin_record({});
	get_current_scene_commandbuffer().begin_record({});
	get_current_ui_commandbuffer().begin_record({});
}

void vulkan_application::end_frame(bool _immediately)
{
	const vulkan_commandbuffer& scene_commandbuffer = get_current_scene_commandbuffer();
	scene_commandbuffer.end_record();
	const vulkan_commandbuffer& ui_commandbuffer = get_current_ui_commandbuffer();
	ui_commandbuffer.end_record();

	const vulkan_commandbuffer& commandbuffer = primary_commandbuffers.at(current_frame);
	std::array<vk::CommandBuffer, 2> excute_commandbuffers = { *(scene_commandbuffer), *(ui_commandbuffer) };
	(*commandbuffer).executeCommands(excute_commandbuffers);
	...
	commandbuffer.submit({ vk::SemaphoreSubmitInfo(acquire_semaphore, {}, vk::PipelineStageFlagBits2::eColorAttachmentOutput) },
		{ vk::SemaphoreSubmitInfo(*(swapchain.get_current_waited_semaphore()), {}, vk::PipelineStageFlagBits2::eColorAttachmentOutput) },
		_immediately);

	try
	{
		vk::PresentInfoKHR present_info = swapchain.present_image();
		acquire_result = present_queue.get_queue().presentKHR(present_info);
	}
	catch (vk::OutOfDateKHRError e)
	{
#ifndef NDEBUG
...
const vulkan_queue& vulkan_application::get_queue(vk::QueueFlagBits _queue_type) const noexcept
{
	switch (_queue_type)
	{
...
const vulkan_commandbuffer& vulkan_application::get_current_scene_commandbuffer() const noexcept
{
	return secondary_commandbuffers.at(current_frame * 2);
}

const vulkan_commandbuffer& vulkan_application::get_current_ui_commandbuffer() const noexcept
{
	return secondary_commandbuffers.at(current_frame * 2 + 1);
}
