...
template<typename Func>
	requires std::predicate<Func, const vk::raii::PhysicalDevice&>
inline uint32_t vulkan_physical_device::create(const vk::raii::Instance& _instance, const std::span<const char* const> _extensions, Func&& _features, vk::SurfaceKHR _surface)
{
	auto all_physical_devices = _instance.enumeratePhysicalDevices();
	auto filtered_physical_devices = all_physical_devices
		| std::views::filter([&](const auto& _physical_device)
			{
				...
				bool has_all_required_extensions = std::ranges::all_of(_extensions,
