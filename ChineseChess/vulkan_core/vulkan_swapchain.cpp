...
void vulkan_swapchain::create(const vk::raii::Instance& _instance, const vk::raii::PhysicalDevice& _physical_device,
	const vk::raii::Device& _device, vk::SurfaceKHR _surface, uint32_t _width, uint32_t _height)
{
...
void vulkan_swapchain::recreate(const vk::raii::PhysicalDevice& _physical_device, const vk::raii::Device& _device, uint32_t _width, uint32_t _height)
{
	...
	vk::SwapchainCreateInfoKHR swapchain_create_info({}, surface, max_index, format,
		vk::ColorSpaceKHR::eSrgbNonlinear, extent, 1, vk::ImageUsageFlagBits::eColorAttachment | vk::ImageUsageFlagBits::eTransferDst, vk::SharingMode::eExclusive,
...
std::pair<vk::Result, vk::Semaphore> vulkan_swapchain::acquire_next_image()
{
	...
	return std::pair<vk::Result, vk::Semaphore>(result.first, *(present_used.at(current_index)));
}
...
const vk::raii::SwapchainKHR& vulkan_swapchain::get_swapchain() const
{
	return swapchain;
}

vk::Extent2D vulkan_swapchain::get_extent() const
{
	return extent;
}

vk::Format vulkan_swapchain::get_format() const
{
	return format;
}

const vk::Image vulkan_swapchain::get_current_image() const
{
	return images.at(current_index);
}

const vk::raii::ImageView& vulkan_swapchain::get_current_imageview() const
{
	return imageviews.at(current_index);
}

const vk::raii::Semaphore& vulkan_swapchain::get_current_waited_semaphore() const
{
