...
void chess_piece::create(const vulkan_application* _app, vk::SampleCountFlagBits _multisample_count, vk::Format _color_formats, vk::Format _depth_format)
{
	...
	auto spirv_code = shader_compiler::get_shader_compiler().compile_shader_to_spv(std::string(SHADERS_PATH) + "chess_piece.slang", { "vertMain", "fragMain" });
	if (spirv_code.empty())
	{
	...
	if (!model_loader::get_model_loader().load_model_file(std::string(MODELS_PATH) + "chess_piece.glb", vertices, indices))
	{
	...
	for (uint32_t i = 0; i < vulkan_common::MAX_FRAMES_IN_FLIGHT; i++)
	{
		...
		buffer.create(_app->get_physical_device(), _app->get_device(), sizeof(chess_piece::UBO), vk::BufferUsageFlagBits::eUniformBuffer, vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent);
...
void chess_piece::resize(const vulkan_application* _app, uint32_t _width, uint32_t _height)
{
	// begin a commandbuffer
	vulkan_commandbuffer commandbuffer = std::move(_app->create_commandbuffers(vk::QueueFlagBits::eGraphics, 1).front());
	commandbuffer.begin_record(vk::CommandBufferUsageFlagBits::eOneTimeSubmit);
	...
	{
		vk::DescriptorBufferInfo buffer_info(ubos.at(i).get_buffer(), 0, sizeof(chess_piece::UBO));
...
void chess_piece::render(const vk::raii::CommandBuffer& _commandbuffer)
{
