...
调用点约定:`{}`(设备侧:图像 / BLAS+TLAS / SBT / SSBO)、`eHostAccessSequentialWrite`(每帧 UBO、staging)、`eHostAccessRandom`(回读)。`vulkan_buffer` 按创建时的 flags 决定 `get_buffer_address()` 联合体的活跃成员(`hostAddress` / `deviceAddress`),调用方必须自行取对应成员。
...
30. ~~**vcpkg baseline 升级后 VMA 头布局**~~(结论 2026-09-11: 不改 vcpkg、不改 CMake,采用**手动修改文件**——把 vcpkg 安装目录 `vulkan-memory-allocator-hpp/vk_mem_alloc.hpp` 里的 `vk_mem_alloc.h` 改为 `vma/vk_mem_alloc.h`。编译期即报 C1083 并带提示,故不再加固;完整表述见 `README.md` §已知限制与取舍)。
