...
`vulkan_semaphore` = 时间线信号量 + 原子 CPU 计数器;`vulkan_recycle_bin` 将资源与 CPU 计数配对,`release()` 在 GPU 越过该值后销毁。CB 在 submit 时 signal、begin_record 时 wait。帧循环无 `vkDeviceWaitIdle`(仅销毁/resize 调用)。Debug 回收日志由 `output = false` 常量经 `if constexpr` 编译期关闭(2026-08-26;实际声明为 `constexpr inline static bool output = false`,`vulkan_recycle_bin.h:29`,2026-09-01 提升 constexpr)。

### 析构顺序加固 (2026-08-27)
默认析构 = 成员声明逆序,命令缓冲/描述符集/allocator/swapchain 生命期约束声明顺序已满足,此处只修"引用先销毁"隐患:
- **`~vulkan_application()`**: 先 `wait()`(内部 `(*device).waitIdle()`,`vulkan_application.cpp:172-175`;**无"device 未创建则跳过"守卫**——`create()` 中途异常时部分构造对象析构会对 null raii Device 调 waitIdle,见 #34),再 `descriptor.clear_descriptor_info()` 销毁描述符池/集——解除对 scene/ui `render_output`、`ocio_images`/`ocio_samplers`、`image_sampler` 的引用,随后成员逆序析构才安全。
...
`VK_KHR_swapchain`, `VK_KHR_synchronization2`, `VK_KHR_acceleration_structure`, `VK_KHR_ray_tracing_pipeline`, `VK_KHR_deferred_host_operations`, `VK_KHR_buffer_device_address`, `VK_KHR_push_descriptor`, `VK_KHR_ray_query`

Features:
- Base: `samplerAnisotropy`, `fillModeNonSolid`, `multiDrawIndirect`, `shaderInt64`
- Vulkan 1.4: `pushDescriptor` | 1.3: `dynamicRendering`, `synchronization2`, `shaderIntegerDotProduct`(SPIR-V 1.6 整数点积) | 1.2: `bufferDeviceAddress`, `runtimeDescriptorArray`, `scalarBlockLayout`, `timelineSemaphore` | 1.1: `shaderDrawParameters`
...
扫描基线:2026-08-08(CMake 迁移 + scene_light 重构);Re-scanned 2026-08-12(dirty-gating + TLAS refit,findings 14-23)、2026-08-15(28-31)、2026-08-16(#29 解决)、2026-08-18(#18 修复)、2026-08-26(#20 修复,新增 #32/#33)、2026-08-27(#32 路径更新、#33 解决)、2026-08-28(#3 引用修正、#34 现状记录、缓存文件形态/路径修正、swapchain noexcept 保留确认)、2026-08-29(#5 修正、#8 改判 FIXED、#27 数值修正)。
...
3. **F5 热重载编译错误直接终止**: `scene_manager.cpp:141-144` 仅分发 `recreate()` + `update()`;`recreate()` 先 retire 旧 pipeline/SBT 再创建新的(`scene_raytracing_render.cpp:78-83`、`scene_rasterization_render.cpp:158-162`),编译失败异常无捕获传播 → `std::terminate`。Fix: 先构造,成功再 retire。OPEN。
...
5. **空容器 null 描述符写入**(OPEN,2026-08-29 复核修正): 2026-08-12 记录的"model 路径已修(空场景保持有效 TLAS + 1 条目 dummy SSBO)"与代码不符——代码中不存在 dummy SSBO(全库无该标识),`scene_model_manager::update_tlas()` 在模型为空时直接 `recycle_bin.retire(tlas)`(`scene_model_manager.cpp:247-251`),模型/材质/灯光三路 SSBO 均在容器为空时跳过创建,RT 描述符照常写入(可能为 null 的)缓冲/AS 句柄;`nullDescriptor` 未启用(仅 `VK_KHR_robustness2` 下存在)。模型路径隐患见 #15。
...
8. ~~BLAS/TLAS 构建队列~~(FIXED 2026-08-29 复核: 实际已用 compute 队列): `scene_model_manager::update_meshes()` 与 `update_tlas()` 的命令缓冲均从 `compute_queue` 的命令池创建并以 `compute_queue.get_index()` 提交(`scene_model_manager.cpp:217-223`、`258-262`),BLAS/TLAS 构建不再走 graphics 队列。
...
34. **`~vulkan_application()` 对未创建 device 无守卫调 waitIdle**(现状记录,2026-08-28;作者决定以当前代码为准,暂不修改): `wait()` 无条件 `(*device).waitIdle()`(`vulkan_application.cpp:172-175`);`vulkan_device` 的 raii Device 默认构造为 null(`vulkan_device.h:24`)。正常流程 device 先于析构创建,仅当 `create()` 中途抛异常时,部分构造对象的析构会对 null device 调 waitIdle。析构顺序本身正确(wait 先于 `descriptor.clear_descriptor_info()`)。
